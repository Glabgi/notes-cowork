            ...
            {[1,2,3,4,5,6].map(n => (
              <button
                ...
                className={cn(
                  ...
                  pomodoros === n
                    ? 'bg-[#eef3ee] border-[#a9cdbf] text-[#4a8a78]'
    ...
    >
      <div className="px-2 py-1 h-full flex flex-col justify-start">
        <div className="flex items-start justify-between gap-1">
          ...
          <button
            onClick={onDelete}
            className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-[#f3e0dc] rounded-[4px] transition-all flex-shrink-0"
          >
            <Trash2 size={10} className="text-[#d06a5e]" />
          </button>
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1.5 ml-auto">
            ...
            {blocks.length > 0 && (
              <button
                onClick={clearDay}
                className="px-2.5 py-1.5 text-xs rounded-[8px] bg-[var(--bg-card)] border border-[var(--border)] text-[var(--text-muted)] hover:text-[#d06a5e] hover:border-[#e3a59e] transition-colors duration-150"
              >
                Очистить
              </button>
            )}
          ...
          <div className="w-12 flex-shrink-0">
            {HOURS.map(h => (
              <div key={h} style={{ height: HOUR_HEIGHT }} className="flex items-start justify-end pr-2 pt-0.5">
                <span className="text-[10px] text-[#ddd2c2] tabular-nums font-medium">{h}:00</span>
              </div>
            ))}
          </div>

          {/* Grid */}
          <div className="flex-1 relative border-l border-[#f0ece4]">
            {HOURS.map(h => (
              <div key={h} style={{ height: HOUR_HEIGHT }} className="border-b border-[#f0ece4] relative">
                <div className="absolute left-0 right-0 top-1/2 border-b border-[#faf7f1] border-dashed" />
              </div>
            ))}
            ...
            {isToday && currentHour >= 6 && currentHour <= 24 && (
              ...
              >
                <div className="w-2 h-2 rounded-full bg-[#d06a5e] flex-shrink-0 -ml-1" />
                <div className="flex-1 border-t border-[#d06a5e]/50" />
              </div>
            )}
