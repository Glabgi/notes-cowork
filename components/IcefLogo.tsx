...
/**
 * I.C-E.F · Notes Cowork — horizontal lockup for the top bar.
 * Bespectacled study mark + "I.C-E.F / notes cowork" wordmark (navy theme).
 */
export default function IcefLogo({ size = 40 }: { size?: number }) {
  const INK = '#e7ecf7';  // light text on navy
  const BLUE = '#5b8cff';
  return (
    <span className="inline-flex items-center gap-2.5 group-hover:opacity-90 transition-opacity select-none">
      ...
      <span
        ...
        style={{ width: size, height: size, background: '#1a2540', border: `2.5px solid ${BLUE}` }}
        aria-hidden
      >
      ...
      <span className="hidden sm:flex flex-col leading-none">
        ...
        <span className="font-extrabold tracking-tight text-[13px] -mt-0.5 flex items-baseline gap-1.5 lowercase" style={{ color: BLUE }}>
          notes
